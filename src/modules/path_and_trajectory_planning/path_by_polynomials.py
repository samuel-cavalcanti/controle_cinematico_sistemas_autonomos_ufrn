...
def find_coefficients(initial_position: Position, final_position: Position) -> tuple[list[float], list[float]]:
    ...
    final_angle_is_problematic = is_in_undefined_region(
        final_position.theta_in_rads)
    initial_angle_is_problematic = is_in_undefined_region(
        initial_position.theta_in_rads)
    ...
    if initial_angle_is_problematic and final_angle_is_problematic:
        ...
        b_0 = initial_position.y
        b_1 = 2 * (delta_y - d_f * delta_x) - \
            d_f * a_3  # + B_3, mas B_3 é zero
        ...
        b_0 = initial_position.y
        b_1 = d_i * a_1
        b_2 = 3 * delta_y - 3 * d_f * delta_x + \
            d_f * a_2 - 2 * (d_i - d_f) * a_1
        b_3 = 3 * d_f * delta_x - 2 * delta_y - \
            d_f * a_2 - (2 * d_f - d_i) * a_1
...
def create_path_functions(x_coefficients: list[float], y_coefficients: list[float]) -> \
    ...
    x_function = polynomial_function(
        x_coefficients[0], x_coefficients[1], x_coefficients[2], x_coefficients[3])
    y_function = polynomial_function(
        y_coefficients[0], y_coefficients[1], y_coefficients[2], y_coefficients[3])
...
def path_creator(initial_position: Position, final_position: Position) -> tuple[Callable, Callable, Callable]:
    x_coefficients, y_coefficients = find_coefficients(

...
def get_motors(client_id: int):
    template_string = 'Dynamic{}Joint'
    motors_id = [template_string.format(side) for side in ['Left', 'Right']]
    left = coppeliasim.get_object(client_id, motors_id[0])
    right = coppeliasim.get_object(client_id, motors_id[1])
